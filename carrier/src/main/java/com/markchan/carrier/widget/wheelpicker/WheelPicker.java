 ...
 * @author AigeStudio 2016-06-17 更新项目结构 <p> New project structure
 * @version 1.1.0
 */
public class WheelPicker extends View implements IWheelPicker, Runnable {
    ...
    /**
     * Determines whether the current scrolling animation is triggered by touchEvent or
     * setSelectedItemPosition. User added eventListeners will only be fired after touchEvents.
     */
    ...
    public WheelPicker(Context context, AttributeSet attrs) {
        ...
        mData = Arrays.asList(getResources()
                .getStringArray(idData == 0 ? R.array.wheel_picker_default_array : idData));
        mItemTextSize = a.getDimensionPixelSize(R.styleable.WheelPicker_wheel_item_text_size,
                getResources().getDimensionPixelSize(R.dimen.wheel_picker_item_default_text_size));
        ...
        mItemSpace = a.getDimensionPixelSize(R.styleable.WheelPicker_wheel_item_space,
                getResources().getDimensionPixelSize(R.dimen.wheel_picker_item_default_space));
        ...
        mIndicatorSize = a.getDimensionPixelSize(R.styleable.WheelPicker_wheel_indicator_size,
                getResources().getDimensionPixelSize(R.dimen.wheel_picker_indicator_size));
    ...
    public void setSelectedItemPosition(int position, final boolean animated) {
        ...
        if (animated && mScroller
                .isFinished()) { // We go non-animated regardless of "animated" parameter if scroller is in motion
            int length = getData().size();
            int itemDifference = position - mCurrentItemPosition;
            if (itemDifference == 0) {
                return;
            }
            if (isCyclic && Math.abs(itemDifference) > (length
                    / 2)) { // Find the shortest path if it's cyclic
     ...
     * 滚轮选择器Item项被选中时监听接口
     *
     * @author AigeStudio 2016-06-17 新项目结构
     * @version 1.1.0
     */
     ...
     * 滚轮选择器滚动时监听接口
     *
     * @author AigeStudio 2016-06-17 新项目结构 <p> New project structure
     * @since 2016-06-17
     */
    public interface OnWheelChangeListener {
         ...
         * position in list
         *
         * @param position 当前选中的数据项在数据列表中的位置 <p> Current selected item data's position in list
         */
